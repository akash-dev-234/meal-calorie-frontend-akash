...
export function AuthForm({ mode }: Props) {
  ...
  const copy = {
    subtitle: isRegister ? "Create your account to get started" : "Sign in to your account",
    title: isRegister ? "Create account" : "Welcome back",
    description: isRegister ? "Fill in your details below" : "Enter your credentials to continue",
    submit: isRegister ? "Create account" : "Sign in",
    submitting: isRegister ? "Creating account…" : "Signing in…",
    passwordPlaceholder: isRegister ? "Min. 8 characters" : "••••••••",
    switchPrompt: isRegister ? "Already have an account?" : "Don't have an account?",
    switchLabel: isRegister ? "Sign in" : "Create one",
  ...
  const onSubmit = async (values: RegisterInput | LoginInput) => {
    ...
    try {
      ...
      setAuth(res.token, user as User)
      router.push("/dashboard")
    } catch (err) {
      if (err instanceof ApiError) {
        setServerError(AUTH_ERRORS[err.status] ?? err.message)
      } else {
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  tabIndex={-1}
                >
                  {showPassword ? <EyeOff className="h-4 w-4 mb-2" /> : <Eye className="h-4 w-4 mb-2" />}
                </button>
              </div>
          ...
          <CardFooter className="flex flex-col gap-4">
            ...
            <p className="text-sm text-muted-foreground text-center">
              {copy.switchPrompt}{" "}
              <Link href={copy.switchHref} className="font-medium text-primary hover:underline underline-offset-4">
                {copy.switchLabel}
              </Link>
            </p>
          </CardFooter>
